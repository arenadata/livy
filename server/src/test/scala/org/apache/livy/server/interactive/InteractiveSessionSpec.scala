...
class InteractiveSessionSpec
  ...
  private def createSession(
    ...
    InteractiveSession.create(
      0, None, null, None, livyConf, accessManager, req, sessionStore, None, mockApp)
  }
  ...
  describe("A spark session") {
    ...
    it("should get scala version matched jars with livy.repl.jars") {
      ...
      assert(
        properties(LivyConf.SPARK_JARS).split(",").toSet === Set(
          "test_2.10-0.1.jar",
          "local://dummy-path/test/test1_2.10-1.0.jar",
          "hdfs:///dummy-path/test/test3.jar",
          "dummy.jar"
      ...
      assert(
        properties1(LivyConf.SPARK_JARS).split(",").toSet === Set(
          "file:///dummy-path/test/test2_2.11-1.0-SNAPSHOT.jar",
          "hdfs:///dummy-path/test/test3.jar",
          "dummy.jar"
        )
    ...
    withSession("should report an error if accessing an unknown variable") { _ =>
    ...
    withSession("should get statement progress along with statement result") { _ =>
      val code =
        """
    ...
    withSession("should error out the session if the interpreter dies") { _ =>
