...
import org.mockito.ArgumentMatchers.{any, anyInt, argThat, eq => eqTo}
...
class FileSystemStateStoreSpec extends AnyFunSpec with LivyBaseUnitTestSuite with Matchers {
  describe("FileSystemStateStore") {
    def pathEq(wanted: String): Path =
      argThat[Path]((p: Path) => p != null && p.toString == wanted)
    ...
    it("set should write with an intermediate file") {
      ...
      verify(fileContext).rename(pathEq("/key.tmp"), pathEq("/key"), eqTo(Rename.OVERWRITE))
      verify(fileContext).delete(pathEq("/.key.tmp.crc"), eqTo(false))
    }
    ...
    it("remove should delete file") {
      ...
      verify(fileContext).delete(pathEq("/key"), org.mockito.ArgumentMatchers.eq(false))
    }
  }
}
